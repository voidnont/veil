...
class GeckoStageAFilesTests(unittest.TestCase):
    ...
    def test_linux_mozconfig_is_full_unofficial_browser_build(self):
        text = self.read("gecko/mozconfig.linux")
        self.assertIn("ac_add_options --enable-application=browser", text)
        self.assertIn("ac_add_options --with-branding=browser/branding/unofficial", text)
        self.assertNotIn("--enable-artifact-builds", text)

    def test_windows_mozconfig_is_full_unofficial_browser_build(self):
        text = self.read("gecko/mozconfig.windows")
        self.assertIn("ac_add_options --enable-application=browser", text)
        self.assertIn("ac_add_options --with-branding=browser/branding/unofficial", text)
        self.assertNotIn("--enable-artifact-builds", text)

    def test_bootstrap_scripts_use_pinned_official_upstream(self):
