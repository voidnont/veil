...
impl RuntimeEventLoop {
    ...
    pub fn pop(&mut self) -> Option<QueuedRuntimeEvent> {
    ...
    pub fn set_timeout(&mut self, delay: Duration, repeat: bool) -> Result<u64, String> {
        if self.timers.len() >= Self::MAX_TIMERS {
            return Err("site timer limit reached".into());
        }
    ...
    pub fn animation_timestamp_ms(&self) -> f64 {
...
mod tests {
    ...
    fn timers_are_bounded() {
        ...
        assert!(
            matches!(loop_.pop().map(|x| x.event), Some(RuntimeEvent::Timer { timer_id }) if timer_id == id)
        );
    }
}
