...
impl SharedBrowserStorage {
    ...
    pub fn apply_script_snapshot(&self, top_level: &Url, snapshot: &ScriptStorageSnapshot) {
        ...
        state
            .local
            .insert(partition.clone(), snapshot.local.clone());
    ...
    pub fn cookie_header_for(
        ...
        let request_path = if request.path().is_empty() {
            "/"
        } else {
            request.path()
        };
        ...
        let mut eligible: Vec<&Cookie> = jar
            .iter()
            .filter(|cookie| {
                ...
                if !domain_matches || !path_matches(request_path, &cookie.path) {
                    return false;
                }
    ...
    pub fn store_set_cookie(&self, top_level: &Url, request: &Url, header: &str) {
        let partition = site_key_for_url(top_level);
        let Some(request_host) = request.host_str().map(|host| host.to_ascii_lowercase()) else {
            return;
        };
        if partition.is_empty() {
            return;
        ...
        let name = name.trim();
        if name.is_empty() || contains_cookie_ctl(name) {
            return;
        }
    ...
    pub fn script_cookie_string(&self, top_level: &Url, document: &Url) -> String {
        let partition = site_key_for_url(top_level);
        let Some(host) = document.host_str().map(|host| host.to_ascii_lowercase()) else {
            return String::new();
        };
        ...
        let Some(jar) = state.cookies.get_mut(&partition) else {
            return String::new();
        };
        jar.retain(|cookie| {
            cookie
...
fn same_site(top_level: &Url, request: &Url) -> bool {
    top_level.scheme() == request.scheme()
        && site_key_for_url(top_level) == site_key_for_url(request)
}
...
mod tests {
    ...
    fn cookies_are_partitioned_by_top_level_site() {
        ...
        storage.store_set_cookie(
            &a,
            &cdn,
            "sid=one; Domain=example; Path=/; Secure; SameSite=None",
        );
        assert_eq!(
            storage.cookie_header_for(&a, &cdn, false, true).as_deref(),
            Some("sid=one")
        );
    ...
    fn samesite_strict_blocks_cross_site_context() {
        ...
        assert!(storage
            .cookie_header_for(&top, &cross, true, true)
            .is_none());
    }
    ...
    fn host_prefix_requires_secure_host_only_root_cookie() {
        ...
        assert_eq!(
            storage.cookie_header_for(&url, &url, true, true).as_deref(),
            Some("__Host-id=ok")
        );
    }
